// src/pages/Usuario/EditarUsuario.tsx
...
export default function EditarUsuario() {
  ...
  // Form (mismos nombres que el backend)
  const [Nombre_Usuario, setNombre_Usuario] = useState("");
  const [Correo, setCorreo] = useState("");
  const [Contraseña, setContraseña] = useState(""); // opcional (si se envía, backend la actualiza)
  ...
  useEffect(() => {
    ...
    const cargarTodo = async () => {
      ...
      try {
        ...
        const u: Usuario = uRes.data;

        setNombre_Usuario(u.Nombre_Usuario || "");
        setCorreo(u.Correo || "");
        setFoto_Perfil(u.Foto_Perfil || "");
        setID_Empleado(
          typeof u.ID_Empleado === "number" ? u.ID_Empleado : u.ID_Empleado ? Number(u.ID_Empleado) : ""
        );
        setID_Rol(u.ID_Rol || "");
        setEstado((u.Estado as "Activo" | "Inactivo") || "Activo");
      } catch (err: any) {
        setMsg(err?.response?.data?.message || "Error al cargar datos");
      } finally {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (!Nombre_Usuario.trim() || !Correo.trim() || !ID_Rol) {
      setMsg("Completa: Usuario, Correo y Rol.");
      return;
    }
    ...
    try {
      ...
      // Armamos payload: solo enviamos Contraseña si el usuario escribió algo
      const payload: any = {
        Nombre_Usuario,
        Correo,
      ...
      setMsg(err?.response?.data?.message || "Error al actualizar usuario");
      setCargandoFoto(false);
    } finally {
      setCargando(false);
    }
    ...
    <div className="card" style={{ padding: 24 }}>
      <div className="hdr">
        <div>
          ...
          <p className="subtitle">Actualiza la información de la cuenta</p>
        </div>
